...
def default_edit() -> dict:
    return {
        "prefix": PREFIXES[5], "suffix": SUFFIXES[5], "title": "", "font": "bold",
        "underline": False, "spacing": "compact", "separator": " ", "ornament": "",
        "marks": "", "crown": False, "prefix_on": True, "suffix_on": True,
        "emoji": False, "emoji_char": "✨",
    }


def default_live() -> dict:
    return {
        "prefix_on": True, "suffix_on": True, "underline": False, "emoji": True,
        "emoji_char": "🔥", "font": "bold", "spacing": "compact", "crown": False,
        "prefix": PREFIXES[1], "suffix": SUFFIXES[1], "ornament": "", "marks": "",
    }
...
def session(context: ContextTypes.DEFAULT_TYPE) -> dict:
    ...
    d.setdefault("live", default_live())
...
def menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [btn("Bulk Names", callback_data="menu|bulk", style="primary", icon=EMOJI_IDS[2]), btn("Help", callback_data="menu|help", style="success", icon=EMOJI_IDS[0])],
        [btn("Music Bot", url=MUSIC_URL, style="primary", icon=EMOJI_IDS[3]), btn("Bio Style", url=STYLE_URL, style="success", icon=EMOJI_IDS[1])],
        [btn("Owner", url=OWNER_URL, style="danger", icon=EMOJI_IDS[4]), btn("Channel", url=SUPPORT_URL, style="danger", icon=EMOJI_IDS[2])],
    ])


def fsub_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [btn("Join TG_BIO_STYLE", url=STYLE_URL, style="primary", icon=EMOJI_IDS[4])],
        [btn("Verify / Try Again", callback_data="fsub|ok", style="success", icon=EMOJI_IDS[1])],
        [btn("Music Bot", url=MUSIC_URL, style="primary", icon=EMOJI_IDS[3]), btn("Owner", url=OWNER_URL, style="danger", icon=EMOJI_IDS[4])],
    ])
...
def live_kb(parts: dict, rows: list[str], page: int) -> InlineKeyboardMarkup:
    def flag(key: str) -> str:
        return "ON" if parts.get(key) else "OFF"

    base = styles_kb(rows, page).inline_keyboard
    controls = [
    ...
    return InlineKeyboardMarkup(controls + base)
...
async def is_member(bot, user_id: int) -> bool:
    if not storage.fsub_enabled() or not FORCE_CHANNEL or user_id in ADMIN_IDS or user_id == OWNER_ID:
        return True
    try:
...
async def ask_fsub(target) -> None:
    await target.reply_text(
        ...
        parse_mode="HTML", reply_markup=fsub_kb(),
    )
...
async def open_live(target, data: dict, edit: bool = False) -> None:
    parts = data["live"]
    rows = live_designs(data["name"], parts)
    data["rows"], data["category"] = rows, "live"
    data["page"] = min(max(data.get("page", 0), 0), max((len(rows) - 1) // PAGE_SIZE, 0))
    pages = max((len(rows) + PAGE_SIZE - 1) // PAGE_SIZE, 1)
    flag = lambda key: "ON" if parts.get(key) else "OFF"
    text = (
        f"<b>🎬 LIVE DESIGN STUDIO</b>\n<b>Name:</b> {html.escape(data['name'])}\n"
        f"<b>Page:</b> {data['page'] + 1}/{pages}\n\n"
        f"Prefix <b>{flag('prefix_on')}</b> • Suffix <b>{flag('suffix_on')}</b> • "
        f"Underline <b>{flag('underline')}</b> • Emoji <b>{flag('emoji')}</b>\n"
        f"Font <b>{html.escape(parts.get('font', 'bold'))}</b>\n\n"
        f"{pe(0)} <b>Controls upar se change karo — designs live update honge:</b>"
    )
...
async def open_styles(target, data: dict, edit: bool = False) -> None:
    if data.get("category") == "live":
        await open_live(target, data, edit=edit)
        return
...
async def on_private_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if mode == "wait_bio":
        ...
        await open_rows(update.message, data, bios(data["name"]), "bio")
        return
    if mode == "wait_bulk":
    ...
    await update.message.reply_text(
        f"<b>Name:</b> {html.escape(data['name'])}\n{pe(0)} Kripya niche 4 category se select karein:",
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if payload == "menu|home":
        data["mode"] = "menu"; await query.message.reply_text(welcome_html(), parse_mode="HTML", reply_markup=menu_kb()); return
    if payload == "menu|name":
        data["mode"] = "wait_name"; await query.message.reply_text("Kripya wo name enter karein jo aap design karwana chahte hain:"); return
    if payload == "menu|bio":
        data["mode"] = "wait_bio"; await query.message.reply_text("Bio ke liye naam / text bhejo:"); return
    if payload == "menu|bulk":
        data["mode"] = "bulk_n"; await query.message.reply_text("Ek saath multiple names stylish bana sakte ho.\nKitne names? (2-10)", reply_markup=bulk_count_kb()); return
    if payload == "menu|help":
        await query.message.reply_text(help_html(), parse_mode="HTML", reply_markup=menu_kb()); return
    if payload.startswith("bulk|"):
        data["bulk_n"], data["mode"] = int(payload.split("|")[1]), "wait_bulk"
    ...
    if payload.startswith("cat|"):
        data["category"], data["page"], data["mode"] = payload.split("|", 1)[1], 0, "styles"
    ...
    if payload.startswith("live|"):
        ...
        if key == "prefix": parts["prefix_on"] = not parts.get("prefix_on")
            ...
            parts.update({
                "prefix_on": True, "suffix_on": True, "underline": True, "emoji": True,
                "emoji_char": "🔥", "font": "boldscript", "spacing": "wide", "crown": True,
                "prefix": PREFIXES[11] if len(PREFIXES) > 11 else PREFIXES[1],
                "suffix": SUFFIXES[11] if len(SUFFIXES) > 11 else SUFFIXES[1],
                "ornament": "♡", "marks": "✦",
            })
        data["category"], data["page"] = "live", 0
        await open_live(query.message, data, edit=True)
        return
    if payload.startswith("page|"):
        ...
        if act == "next": data["page"] = min(data["page"] + 1, pages - 1)
            ...
            if data.get("page", 0) <= 0:
                data["mode"] = "pick_cat"
                await query.message.edit_text(f"<b>Name:</b> {html.escape(data['name'])}\n{pe(0)} Category select karo:", parse_mode="HTML", reply_markup=cat_kb())
                return
    ...
    if payload == "act|copy":
        ...
        if text:
            if CopyTextButton is None:
                ...
                await query.message.reply_text(f"{pe(3)} <b>Copy your final design</b>", parse_mode="HTML", reply_markup=InlineKeyboardMarkup([[btn("Copy Final Design", copy_text=CopyTextButton(text=text), style="success", icon=EMOJI_IDS[3])]]))
        return
    if payload == "act|edit":
    ...
    if payload == "act|another":
        ...
        await open_styles(query.message, data, edit=True); return
    if payload == "act|done":
        preview = compose(data["name"], data["edit"]); data["picked"] = preview
...
def is_admin(user) -> bool:
    ...
    return user.id in ADMIN_IDS or user.id == OWNER_ID or username == OWNER_USERNAME


async def fsub_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = update.effective_user
    if not is_admin(user):
        await update.message.reply_text("Ye command sirf owner/admin ke liye hai.")
        return
